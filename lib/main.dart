...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      home: Scaffold(
        appBar: AppBar(
          title: const Text('Opacity Animation between Leaves'),
        ),
...
class _OpacityAnimationBetweenLeavesState
  ...
  void initState() {
    ...
    _opacityAnimation = Tween<double>(begin: 0.0, end: 1.0).animate(
      CurvedAnimation(
        parent: _controller,
        curve: const Interval(0.5, 1.0, curve: Curves.easeInOut), // Delay the appearance of the second leaf by 0.5 seconds
      ),
    );
...
class LeafPainter extends CustomPainter {
  ...
  LeafPainter({required this.opacityAnimation}) : super(repaint: opacityAnimation);

  @override
  void paint(Canvas canvas, Size size) {

     canvas.translate(size.width / 2, size.height / 2-120); // Move to the center of the canvas
    ...
    path2.quadraticBezierTo(
        size.width * 0.15, size.height * 0.5, size.width -150, size.height+20);
